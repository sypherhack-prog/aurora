                    ...
                    <tbody className="divide-y divide-zinc-800">
                        {subscriptions.map((sub) => (
                            <tr key={sub.id} className="hover:bg-zinc-800/30 transition">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap"><span className="text-sm text-zinc-400 font-mono">{sub.payments[0]?.mvolaRef || '—'}</span></td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-zinc-400">{sub.payments[0]?.phoneNumber || '—'}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-zinc-400">{new Date(sub.createdAt).toLocaleDateString('fr-FR')}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-right"><SubscriptionActions subscriptionId={sub.id} status={sub.status} /></td>
                            </tr>
                        ))}
